...
let makeMove = () => {
  let grid = getSimpleGrid(a)
  let gridmoves = MOVES.map(dir => moveGrid(grid, dir))
  let numzeros = gridmoves.map(posgrid => count0(posgrid))
  let goodmoves = max(numzeros)
  let move = rand(0, goodmoves.length)
  a.move(move)
